# Chapter 6: Observability & Chargeback
...
```bash
oc apply -f chapter-6/load-generator-configmap.yml
```

```bash
export ROUTE_HOST=$(oc get route openshift-ai-inference -n openshift-ingress -o jsonpath='{.spec.host}')
envsubst < chapter-6/load-generator-job.yml | oc apply -f -
```
...
```bash
oc apply -f chapter-6/coo-subscription.yml
```
...
```bash
oc apply -f chapter-6/perses-ui-plugin.yml
```
...
```bash
oc apply -f chapter-6/perses-datasource.yml
```
...
```bash
oc apply -f chapter-6/perses-dashboard.yml
```
